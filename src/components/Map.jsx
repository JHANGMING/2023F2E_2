...
export const Map=()=>{
  ...
  const handleMouseOver = (event) => {
    d3.select(event.currentTarget)
      ...
      const countyName = event.currentTarget.getAttribute('data-name');
  ...
  const handleMouseOut = (event) => {
    ...
    const element = document.querySelector(`[data-name="${areaName}"]`);
      if (element) {
        setShowReset(true);
      ...
      svg.transition()
        .duration(750)
        .attr("viewBox", 
          `${centerX - svgWidth / scale / 2} ${centerY - svgHeight / scale / 2} ${svgWidth / scale} ${svgHeight / scale}`
        );
      }
    };
   ...
   const handleClick = (event) => {
    ...
    dispatch(saveCityName(countyName))
    zoomToArea(countyName);
    };
